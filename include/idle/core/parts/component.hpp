...
#ifndef IDLE_CORE_PARTS_COMPONENT_HPP_INCLUDED
#define IDLE_CORE_PARTS_COMPONENT_HPP_INCLUDED
...
namespace idle {
/// A component is enclosed in another service and used by it exclusively
template <typename Child>
class Component : public Import, public Inplace<Child> {
  static_assert(is_service<Child>::value,
                "Can only use a Service as a component! "
                "If you are trying to depend on an Interface use "
                "idle::Dependency instead!");

public:
  template <typename... Args>
  explicit Component(Service& owner, Args&&... args)
    : Import(owner)
    , Inplace<Child>(Inheritance::weak(*static_cast<Import*>(this)),
...
#endif // IDLE_CORE_PARTS_COMPONENT_HPP_INCLUDED
